-- 20260513210000_approve_promotes_shadow_to_group.sql
-- approve_node_claim Path A (claim existing relative) now correctly promotes
-- the target row into the target group's scope. Previously the row's
-- family_group_id was left untouched, so claiming a shadow (which lives in
-- the anchor user's personal NULL scope) left the row out-of-scope. The
-- validate_relative_id_in_tree trigger then refused the membership insert.
--
-- Fix: also update family_group_id + clear mirrors_relative_id on the
-- target. For already-in-scope rows (the common case), this is a no-op
-- for those columns. For Path B (newly inserted row), the INSERT already
-- sets family_group_id = v_claim.group_id and mirrors_relative_id = NULL,
-- so the additional clauses are also a no-op there.
