-- Guard the personal-shadow projection in approve_node_claim against
-- a NULL anchor.gender. Without this, the inverse-relationship CASE
-- returns NULL and the INSERT into relatives fails on the NOT NULL
-- relationship_type column, aborting the whole approval transaction.
...
BEGIN
  ...
  IF v_claim.declared_edge_path IN ('parent', 'child', 'spouse', 'sibling') THEN
    ...
    -- v_anchor_relative.gender being NULL would null the inverse-relationship
    -- CASE result, which would crash on relatives.relationship_type NOT NULL.
    -- Skip the shadow in that case; the rest of the approval still completes.
    IF v_anchor_relative.id IS NOT NULL
       AND v_claimant_personal_self_id IS NOT NULL
       AND v_anchor_relative.gender IS NOT NULL THEN
